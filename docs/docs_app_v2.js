...
const PLANS = {
  one: { label: "Documento (9 €)", key: "one" },
  annual: { label: "Pro anual (49 €)", key: "annual" },
};
// Helpers
const EUR = (n) => {
...
const els = {
  ...
  buyOneBtn: document.getElementById("buyOneBtn"),
  buyAnnualBtn: document.getElementById("buyAnnualBtn"),
...
function focusResultsCard() {
  ...
  const card = anchor?.closest(".hero-card");
  if (!card) return;
...
async function startCheckout(planKey) {
  try {
    ...
    setStatus("Error iniciando el pago. Revisa CORS/Worker.", "warn");
    return false;
  }
}
...
els.unlockBtn.addEventListener("click", unlockWithCode);
  ...
  // Pago (Stripe Checkout vía Worker)
  els.buyOneBtn?.addEventListener("click", () => startCheckout("one"));
  els.buyAnnualBtn?.addEventListener("click", () => startCheckout("annual"));
  // Compatibilidad si existiera un buyBtn legacy
